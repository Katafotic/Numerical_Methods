...
if __name__ == '__main__':
    ...
    for i in parameter:
        ...
        #class LU in documentation
        #https://docs.roguewave.com/en/imsl/pynl/current/docs/generated/generated/imsl.linalg.LU.solve.html
        ...
        condition_number = LU_System_Solution.cond()
        np.linalg.norm(delta_A, ord=1)
        solve = LU_System_Solution.solve(delta_B)
        print_array('Solve vector: ', solve, subject='vector')

        r = delta_B - np.matmul(delta_A, delta_B)
        ...
        print('delta_x/x <= ', supremum_delta_X)
...
def get_value(filename, dimension=dimension, subject='matrix') -> float:
    ...
    with open(filename, 'r') as input:
        if (subject == 'matrix') :
            ...
            #print('Vector B:', vector, end='\n\n')
            #writeMatrix("Vector B:", vector)
...
def print_array(title:str, array, subject:str, dimension:int = dimension):
    ...
    # Массив numpy не итерируемый объект. Его нельзя перебрать исп. for и shape
